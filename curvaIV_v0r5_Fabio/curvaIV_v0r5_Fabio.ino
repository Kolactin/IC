...
int dt_CAPdc = 10; // delay (ms) para descarregar capacitor no discharge()
...
boolean flag = true;
int IVcontrol = 3;
int CAPcontrol = 4; // saída digital do arduino que estabelece conexão com o capacitor.
...
//  Serial.print("Tensao media: ");
  ...
  StopTracking(mediaTensaoValor);
  serial.print("Fim de tracking!");
}
...
void loop() {
  ...
  if (number == 1) {
    ...
    while (indice < Ndados){
      Serial.print(indice);
      Serial.print("pressione 1 para iniciar tracking");
      IVadjust();
      delay(dt_medidas);
      GetLightInt();
      GetModTemp(indice);
